...
```text
@authyon/auth   0.2.0-beta.2
@authyon/server 0.2.0-beta.2
```
...
2. Crie a tag anotada `v0.2.0-beta.2` no commit validado.
...
O workflow deriva a versão da tag, valida SemVer e usa o identificador do prerelease como dist tag. Assim, `v0.2.0-beta.2` publica com `--tag beta`; uma versão sem prerelease publica com `--tag latest`.
...
- Próxima correção ou feedback: `0.2.0-beta.3`.
